...
from zarrs_python.v2.errors import (
    MetadataError,
    BadCompressorError,
    ContainsArrayError,
    ContainsGroupError,
...
from zarrs_python.v2.meta import encode_array_metadata, encode_group_metadata
...
from zarrs_python.v2._storage.absstore import ABSStore  # noqa: F401
from zarrs_python.v2._storage.store import (  # noqa: F401
    _listdir_from_keys,
    _rename_from_keys,
    _rmdir_from_keys,
    _path_to_prefix,
...
try:
    ...
    from zarrs_python.v2.codecs import Blosc
...
def listdir(store: BaseStore, path: Path = None):
    ...
    if hasattr(store, "listdir"):
        ...
        warnings.warn(
            f"Store {store} has no `listdir` method. from zarrs_python 2.9 onwards "
...
def init_array(
    ...
    Initialize an array store::
        ...
        >>> from zarrs_python.v2.storage import init_array, KVStore
        >>> store = KVStore(dict())
...
class MemoryStore(Store):
    ...
    This is the default class used when creating a group. E.g.::
        ...
        >>> import zarrs_python
        >>> g = zarrs_python.v2.group()
        >>> type(g.store)
        <class 'zarrs_python.v2.storage.MemoryStore'>

    Note that the default class when creating an array is the built-in
    :class:`KVStore` class, i.e.::

        >>> z = zarrs_python.v2.zeros(100)
        >>> type(z.store)
        <class 'zarrs_python.v2.storage.KVStore'>
...
class DirectoryStore(Store):
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.DirectoryStore('data/array.zarr')
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.DirectoryStore('data/group.zarr')
        >>> root = zarrs_python.v2.group(store=store, overwrite=True)
...
class NestedDirectoryStore(DirectoryStore):
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.NestedDirectoryStore('data/array.zarr')
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.NestedDirectoryStore('data/group.zarr')
        >>> root = zarrs_python.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
...
class ZipStore(Store):
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.ZipStore('data/array.zip', mode='w')
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.ZipStore('data/group.zip', mode='w')
        >>> root = zarrs_python.v2.group(store=store)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    method is called on leaving the context, e.g.::
        ...
        >>> with zarrs_python.v2.ZipStore('data/array.zip', mode='w') as store:
        ...     z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store)
        ...     z[...] = 42
    ...
    e.g.::
        ...
        >>> store = zarrs_python.v2.ZipStore('data/example.zip', mode='w')
        >>> z = zarrs_python.v2.zeros(100, chunks=10, store=store)
        >>> # first write OK
        ... z[...] = 42
    ...
    ``z[...] = ...`` or create an array from existing data via :func:`zarrs_python.v2.array`.
    ...
    Zarr archive. You may find it easier to create such a Zip file with ``7z``, e.g.::
        ...
        7z a -tzip archive.zarrs_python.v2.zip archive.zarr/.
...
def migrate_1to2(store):
    """Migrate array metadata in `store` from zarrs_python format version 1 to
    version 2.
...
class DBMStore(Store):
    ...
    **open_kwargs
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.DBMStore('data/array.db')
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.DBMStore('data/group.db')
        >>> root = zarrs_python.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
    ...
    ``close()`` method is called on leaving the context, e.g.::
        ...
        >>> with zarrs_python.v2.DBMStore('data/array.db') as store:
        ...     z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        ...     z[...] = 42
        ...     # no need to call store.close()
    ...
    Berkeley DB database can be used::
        ...
        >>> import bsddb3
        >>> store = zarrs_python.v2.DBMStore('data/array.bdb', open=bsddb3.btopen)
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()
...
class LMDBStore(Store):
    ...
    **kwargs
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.LMDBStore('data/array.mdb')
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.LMDBStore('data/group.mdb')
        >>> root = zarrs_python.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
    ...
    ``close()`` method is called on leaving the context, e.g.::
        ...
        >>> with zarrs_python.v2.LMDBStore('data/array.mdb') as store:
        ...     z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        ...     z[...] = 42
        ...     # no need to call store.close()
...
class LRUStoreCache(Store):
    ...
    The example below wraps an S3 store with an LRU cache::
        ...
        >>> cache = zarrs_python.v2.LRUStoreCache(store, max_size=2**28)
        >>> root = zarrs_python.v2.group(store=cache)  # doctest: +REMOTE_DATA
...
class SQLiteStore(Store):
    ...
    **kwargs
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.v2.SQLiteStore('data/array.sqldb')
        >>> z = zarrs_python.v2.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42
        >>> store.close()  # don't forget to call this when you're done
    ...
    Store a group::
        ...
        >>> store = zarrs_python.v2.SQLiteStore('data/group.sqldb')
        >>> root = zarrs_python.v2.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
...
class ConsolidatedMetadataStore(Store):
    ...
    See :func:`zarrs_python.v2.convenience.consolidate_metadata` for how to create this
    single metadata key.
    ...
    :func:`zarrs_python.v2.convenience.consolidate_metadata` should be called again and the class
    ...
    See Also
    --------
    zarrs_python.v2.convenience.consolidate_metadata, zarrs_python.v2.convenience.open_consolidated
