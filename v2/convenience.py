...
from zarrs_python.v2.creation import array as _create_array
from zarrs_python.v2.creation import open_array
from zarrs_python.v2.errors import CopyError, PathNotFoundError
from zarrs_python.v2.hierarchy import Group
from zarrs_python.v2.hierarchy import group as _create_group
from zarrs_python.v2.hierarchy import open_group
from zarrs_python.v2.meta import json_dumps, json_loads
...
from zarrs_python.v2.util import TreeViewer, buffer_size, normalize_storage_path
...
def open(store: StoreLike = None, mode: str = "a", *, path=None, **kwargs):
    ...
    **kwargs
        Additional parameters are passed through to :func:`zarrs_python.v2.creation.open_array` or
        :func:`zarrs_python.v2.hierarchy.open_group`.

    Returns
    -------
    z : :class:`zarrs_python.v2.core.Array` or :class:`zarrs_python.v2.hierarchy.Group`
        Array or group, depending on what exists in the given store.

    See Also
    --------
    zarrs_python.v2.creation.open_array, zarrs_python.v2.hierarchy.open_group
    ...
    Storing data in a directory 'data/example.zarr' on the local file system::
        ...
        >>> zw = zarrs_python.v2.open(store, mode='w', shape=100, dtype='i4')  # open new array
        >>> zw
        <zarrs_python.v2.core.Array (100,) int32>
        >>> za = zarrs_python.v2.open(store, mode='a')  # open existing array for reading and writing
        >>> za
        <zarrs_python.v2.core.Array (100,) int32>
        >>> zr = zarrs_python.v2.open(store, mode='r')  # open existing array read-only
        >>> zr
        <zarrs_python.v2.core.Array (100,) int32 read-only>
        >>> gw = zarrs_python.v2.open(store, mode='w')  # open new group, overwriting previous data
        >>> gw
        <zarrs_python.v2.hierarchy.Group '/'>
        >>> ga = zarrs_python.v2.open(store, mode='a')  # open existing group for reading and writing
        >>> ga
        <zarrs_python.v2.hierarchy.Group '/'>
        >>> gr = zarrs_python.v2.open(store, mode='r')  # open existing group read-only
        >>> gr
        <zarrs_python.v2.hierarchy.Group '/' read-only>
...
def save_array(store: StoreLike, arr, *, path=None, **kwargs):
    ...
    Save an array to a directory on the file system (uses a :class:`DirectoryStore`)::
        ...
        >>> import numpy as np
        >>> arr = np.arange(10000)
        >>> zarrs_python.v2.save_array('data/example.zarr', arr)
        >>> zarrs_python.v2.load('data/example.zarr')
        array([   0,    1,    2, ..., 9997, 9998, 9999])

    Save an array to a single file (uses a :class:`ZipStore`)::

        >>> zarrs_python.v2.save_array('data/example.zip', arr)
        >>> zarrs_python.v2.load('data/example.zip')
        array([   0,    1,    2, ..., 9997, 9998, 9999])
...
def save_group(store: StoreLike, *args, path=None, **kwargs):
    ...
    :class:`DirectoryStore`):
        ...
        >>> zarrs_python.v2.save_group('data/example.zarr', a1, a2)
        >>> loader = zarrs_python.v2.load('data/example.zarr')
        >>> loader
    ...
    Save several arrays using named keyword arguments::
        ...
        >>> zarrs_python.v2.save_group('data/example.zarr', foo=a1, bar=a2)
        >>> loader = zarrs_python.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
    ...
    Store several arrays in a single zip file (uses a :class:`ZipStore`)::
        ...
        >>> zarrs_python.v2.save_group('data/example.zip', foo=a1, bar=a2)
        >>> loader = zarrs_python.v2.load('data/example.zip')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
        array([   0,    1,    2, ..., 9997, 9998, 9999])
...
def save(store: StoreLike, *args, path=None, **kwargs):
    ...
    Save an array to a directory on the file system (uses a :class:`DirectoryStore`)::
        ...
        >>> import numpy as np
        >>> arr = np.arange(10000)
        >>> zarrs_python.v2.save('data/example.zarr', arr)
        >>> zarrs_python.v2.load('data/example.zarr')
        array([   0,    1,    2, ..., 9997, 9998, 9999])

    Save an array to a Zip file (uses a :class:`ZipStore`)::

        >>> zarrs_python.v2.save('data/example.zip', arr)
        >>> zarrs_python.v2.load('data/example.zip')
        array([   0,    1,    2, ..., 9997, 9998, 9999])
    ...
    :class:`DirectoryStore` and stores arrays in a group)::
        ...
        >>> a1 = np.arange(10000)
        >>> a2 = np.arange(10000, 0, -1)
        >>> zarrs_python.v2.save('data/example.zarr', a1, a2)
        >>> loader = zarrs_python.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: arr_0, arr_1>
        >>> loader['arr_0']
    ...
    Save several arrays using named keyword arguments::
        ...
        >>> zarrs_python.v2.save('data/example.zarr', foo=a1, bar=a2)
        >>> loader = zarrs_python.v2.load('data/example.zarr')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
    ...
    Store several arrays in a single zip file (uses a :class:`ZipStore`)::
        ...
        >>> zarrs_python.v2.save('data/example.zip', foo=a1, bar=a2)
        >>> loader = zarrs_python.v2.load('data/example.zip')
        >>> loader
        <LazyLoader: bar, foo>
        >>> loader['foo']
...
def tree(grp, expand=False, level=None):
    ...
    >>> zarrs_python.v2.copy_all(g1, h5f)
    (5, 0, 800)
    >>> zarrs_python.v2.tree(h5f)
    /
     ├── bar
     │   ├── baz
     │   └── qux
    ...
    See Also
    --------
    zarrs_python.v2.hierarchy.Group.tree
...
def copy_store(
    ...
    >>> import zarrs_python
    >>> store1 = zarrs_python.v2.DirectoryStore('data/example.zarr')
    >>> root = zarrs_python.v2.group(store1, overwrite=True)
    ...
    >>> store2 = zarrs_python.v2.ZipStore('data/example.zip', mode='w')
    >>> zarrs_python.v2.copy_store(store1, store2, log=stdout)
    copy .zgroup
    copy foo/.zgroup
    copy foo/bar/.zgroup
    ...
    (6, 0, 566)
    >>> new_root = zarrs_python.v2.group(store2)
    >>> new_root.tree()
    /
     └── foo
         └── bar
...
def copy(
    ...
    **create_kws
    ...
    Zarr group::
        ...
        >>> zarrs_python.v2.tree(source)
        /
         ├── foo
         │   └── bar
         │       └── baz (100,) int64
         └── spam (100,) int64
        >>> dest = zarrs_python.v2.group()
        >>> from sys import stdout
        >>> zarrs_python.v2.copy(source['foo'], dest, log=stdout)
        copy /foo
        copy /foo/bar
    ...
    ``dry_run=True`` to find out what would happen without actually copying anything::
        ...
        >>> from sys import stdout
        >>> try:
        ...     zarrs_python.v2.copy(source['foo'], dest, log=stdout, dry_run=True)
        ... except zarrs_python.v2.CopyError as e:
        ...     print(e)
        ...
        copy /foo
        copy /foo/bar
        ...
        >>> zarrs_python.v2.copy(source['foo'], dest, log=stdout, if_exists='replace', dry_run=True)
        copy /foo
        copy /foo/bar
        copy /foo/bar/baz (100,) int64
        ...
        (4, 0, 0)
        >>> zarrs_python.v2.copy(source['foo'], dest, log=stdout, if_exists='skip', dry_run=True)
        copy /foo
        copy /foo/bar
        copy /foo/bar/baz (100,) int64
...
def copy_all(
    ...
    **create_kws
    ...
    >>> dest = zarrs_python.v2.group()
    >>> import sys
    >>> zarrs_python.v2.copy_all(source, dest, log=sys.stdout)
    copy /foo
    copy /foo/bar
    copy /foo/bar/baz (100,) int64
    copy /spam (100,) int64
...
def consolidate_metadata(store: BaseStore, metadata_key=".zmetadata", *, path=""):
    ...
    Returns
    -------
    g : :class:`zarrs_python.v2.hierarchy.Group`
