...
from zarrs_python.v2.meta import ZARR_FORMAT, json_dumps, json_loads
from zarrs_python.v2.storage import FSStore
from zarrs_python.v2.storage import NestedDirectoryStore, _prog_ckey, _prog_number, normalize_storage_path
from zarrs_python.v2.storage import array_meta_key as zarr_array_meta_key
from zarrs_python.v2.storage import attrs_key as zarr_attrs_key
from zarrs_python.v2.storage import group_meta_key as zarr_group_meta_key
...
class N5Store(NestedDirectoryStore):
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.N5Store('data/array.n5')
        >>> z = zarrs_python.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42

    Store a group::

        >>> store = zarrs_python.N5Store('data/group.n5')
        >>> root = zarrs_python.group(store=store, overwrite=True)
...
class N5FSStore(FSStore):
    ...
    Store a single array::
        ...
        >>> import zarrs_python
        >>> store = zarrs_python.N5FSStore('data/array.n5', auto_mkdir=True)
        >>> z = zarrs_python.zeros((10, 10), chunks=(5, 5), store=store, overwrite=True)
        >>> z[...] = 42

    Store a group::

        >>> store = zarrs_python.N5FSStore('data/group.n5', auto_mkdir=True)
        >>> root = zarrs_python.group(store=store, overwrite=True)
        >>> foo = root.create_group('foo')
        >>> bar = foo.zeros('bar', shape=(10, 10), chunks=(5, 5))
        >>> bar[...] = 42
...
def group_metadata_to_n5(group_metadata: Dict[str, Any]) -> Dict[str, Any]:
    """Convert group metadata from zarrs_python to N5 format."""
...
def array_metadata_to_n5(array_metadata: Dict[str, Any], top_level=False) -> Dict[str, Any]:
    """Convert array metadata from zarrs_python to N5 format. If the `top_level` keyword argument is True,
